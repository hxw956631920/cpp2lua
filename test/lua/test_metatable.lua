...
-- 一个新建的表是没有对应的元表的
t = {}
local prefix = FontAttribute.setFontColor("Lua测试用例1:", 36)
local title = FontAttribute.setFontColor("测试一个新建的表的元表是否为空:", 37)
print(prefix..title)
...
-- print("Lua测试用例2:测试两个表拥有的共同元表是否相同")
...
-- -- -- ====走元方法===
-- -- -- 小米
-- print("Lua测试用例4:测试访问表没有的字段是否走元表__index方法")
-- print(miShen.name)
...
-- -- 此时设置完了该字段因此不会触发元方法
-- print("Lua测试用例5:测试设置了该表的字段后再去访问是否还走元表的__index方法")
-- miShen.name = "米神"
-- print(miShen.name)

-- -- 测试rawget函数
-- -- 结果显示直接打印为空 说明没有走元表
-- print("Lua测试用例6:测试rawget函数")
-- miShen.name = nil
...
-- end
-- print("Lua测试用例7:测试__newindex方法")
-- miShen.name = "米神"
-- print(miShen.name)

-- -- 测试rawset
-- print("Lua测试用例8:测试rawset")
-- miShen.name = nil
